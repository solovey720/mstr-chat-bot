...
async def send_screenshot_wo_filters_document(call: CallbackQuery, state: FSMContext):
...
# Отправляем скрин без фильтров
async def send_screenshot_wo_filters_report(call: CallbackQuery, state: FSMContext):
    #TODO: продумать логику для репорта
    ...
    await bot.edit_message_text(_(call.message.chat.id)('send_report'), chat_id=call.message.chat.id,
                                message_id=call.message.message_id)
    # await call.answer('Отправляем скриншот отчета...', show_alert=True)
    await create_page(User.get_current().id, {'docID': file_id, 'headless': HEADLESS_MODE, 'docType': 'report'})
    try:
...
def register_handlers_search_and_screen(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(send_screenshot_wo_filters_document, Text(startswith='document:'), state=GetInfo.get_screen)
    dp.register_callback_query_handler(send_screenshot_wo_filters_report, Text(startswith='report:'), state=GetInfo.get_screen)
