...
async def screenshot(options = dict()):
    ...
    await page.waitForSelector('#\\33 054', {'timeout': timeout_long,
                                             'visible': True})
...
async def screenshot_html(options=dict()):
    ...
    # login
    # password
    evt_temp = '2048001' if options.get('docType', 'document') == 'document' else (
        '4001' if options.get('docType', 'document') == 'report' else (
            '3140' if options.get('docType', 'document') == 'dossier' else 'error'))
    evt = options.get('evt', evt_temp)
    timeout_long = options.get('timeout_long', 60000)
    timeout_short = options.get('timeout_short', 3000)
    ...
    browser = await launch(
        {'headless': options.get('headless', False), 'ignoreHTTPSErrors': options.get('ignoreHTTPSErrors', True),
         'defaultViewport': options.get('defaultViewport', {'width': 1920, 'height': 1080})})
    page = await browser.newPage()
    await page.goto(path, {'timeout': timeout_long})
    ...
    selector_2 = '#waitBox > div.mstrmojo-Editor.mstrWaitBox.modal' if (options.get('docType',
                                                                                   ...
                                                                                   options.get('docType',
                                                                                               'document') == 'dossier') else (
        '#divWaitBox' if options.get('docType', 'document') == 'report' else 'ERROR')
