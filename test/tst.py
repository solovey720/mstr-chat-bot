...
docID = '520F150011EB25866E6D0080EF154E9B'
...
async def main():
    ...
    try:
        await page.waitForSelector('#waitBox > div.mstrmojo-Editor.mstrWaitBox.modal', {'timeout':60000, 'visible': True} ) #ждем ухода самой загрузки документа и появления загрузки данных борда
        await page.waitForSelector('#waitBox > div.mstrmojo-Editor.mstrWaitBox.modal', {'timeout':60000, 'hidden': True} ) # ждем пока пропадет окно загрузки данных
